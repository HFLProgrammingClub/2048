...
public class Engine {
	...
	public int swipe(int direction){
		...
		if(buffer[0][0] == -1){
			...
			int random = (int)(Math.random() * emptyCells.size());//get a random integer between 0(inclusive) and the number of empty cells(exclusive)
			int cell = emptyCells.get(random);
	...
	private int[][] rotateBoard(int times, int[][] board){
		int[][] buffer = new int[4][4];//rows then columns, 0,0 at bottom left corner
