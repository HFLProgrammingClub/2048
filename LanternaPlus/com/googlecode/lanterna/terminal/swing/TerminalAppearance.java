...
public class TerminalAppearance {
	...
	public TerminalAppearance(Font normalTextFont, Font boldTextFont, TerminalPalette colorPalette, boolean useBrightColorsOnBold) {
		...
		this.cjkFont = deriveCJKFont('¡'); // Used to be 'æ¡œ'; caused unicode errors.
	}

	private Font deriveCJKFont(char character) {
		final BufferedImage bi = new BufferedImage(5, 5, BufferedImage.TYPE_INT_RGB);
		final int targetWidth = bi.getGraphics().getFontMetrics(normalTextFont).charWidth('W');
		final int startSize = normalTextFont.getSize();
		for (int size = startSize; size > 0; size--) {
			final Font font = new Font("Monospaced", Font.PLAIN, size);
			final int characterWidth = bi.getGraphics().getFontMetrics(font).charWidth(character);
	...
	public String toString() {
		return "TerminalAppearance{" + "normalTextFont=" + normalTextFont + ", boldTextFont=" + boldTextFont + ", colorPalette=" + colorPalette + ", useBrightColorsOnBold=" + useBrightColorsOnBold + '}';
	}
}
